...
pub fn schedule_task_trigger_clean_jobs_event(scheduler: &mut AsyncScheduler<Utc>) {
  scheduler.every(1.day()).run(move || async move {
    let job_id = match DB
      .jobs
      .create(NewJob {
        created_by_id: None,
    ...
    let job = QueueJob::builder()
      .job_id(job_id)
      .job_type(QueueJobType::CleanJobs)
      .build();
