...
pub async fn federate(
  ...
  let object = match &doc.object.activity {
    Some(ac) => match deref_activitypub_ref(&ac.object).await {
      Some(obj) => obj,
      None => return Err(LogicErr::InvalidData),
    },
    None => return Err(LogicErr::InvalidData),
  ...
  match object_type {
    ObjectType::Note => match kind {
      ActivityType::Create => federate_create_note(&doc, object, actor_user).await,
